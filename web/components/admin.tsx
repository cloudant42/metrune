        ...
        <div className="admin-grid">
          ...
          <section className="panel" aria-labelledby="pricing-link-title">
            <div className="panel-header">
              <div><p className="eyebrow">Rates used to price reported usage</p><h2 id="pricing-link-title">Provider and model pricing</h2></div>
...
export function MembersPanel({ members }: { members: Member[] }) {
  ...
  async function remove(member: Member) {
    if (!window.confirm(`Remove ${member.displayName ?? member.email} from this workspace?`)) return;
    await run(() => send(`/api/admin/members/${member.userId}`, "DELETE"));
  }
              ...
              <tr key={member.userId}>
                ...
                <td>
                  <select
                    ...
                    disabled={busy}
                    onChange={event => changeRole(member, event.target.value as Member["role"])}
                  >
                    <option value="viewer">Viewer</option>
                    <option value="analyst">Analyst</option>
                    <option value="admin">Admin</option>
...
export function TeamsPanel({ teams }: { teams: Team[] }) {
  ...
  return (
    <section className="panel" aria-labelledby="teams-title">
      <div className="panel-header">
        <div><p className="eyebrow">Group installations for reporting</p><h2 id="teams-title">Teams</h2></div>
...
export function InstallationsPanel({ installations, teams }: { installations: Installation[]; teams: Team[] }) {
  ...
  return (
    <section className="panel" aria-labelledby="installations-title">
      <div className="panel-header">
        <div><p className="eyebrow">Machines uploading sanitized usage</p><h2 id="installations-title">Installations</h2></div>
        {revokedCount > 0 && (
...
export function SettingsPanel({ settings }: { settings: OrgSettings }) {
  ...
  return (
    <section className="panel" aria-labelledby="settings-title">
      <div className="panel-header">
        <div><p className="eyebrow">How long stored snapshots are kept</p><h2 id="settings-title">Retention</h2></div>
      </div>
...
export function IdentityPanel({ settings }: { settings: OrgSettings }) {
  return (
    <section className="panel" aria-labelledby="identity-title">
      <div className="panel-header">
        <div><p className="eyebrow">Sign-in and session settings</p><h2 id="identity-title">Identity</h2></div>
      </div>
...
export function ClassifierPanel({ classifier, credentials }: { classifier: ClassifierSettings; credentials: ProviderCredential[] }) {
  ...
  const [executionMode, setExecutionMode] = useState<ClassifierSettings["executionMode"]>(classifier.executionMode);
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    const failure = await send("/api/admin/classifier", "PATCH", {
      enabled, executionMode, providerId, endpoint: effectiveEndpoint, model, credentialId, responseMode: detectedResponseMode,
    });
    setBusy(false);
    if (failure) setError(failure);
    else {
      setMessage(executionMode === "managed"
        ? "Managed classifier saved. Clients send bounded semantic text to Metrune; the provider credential stays in this vault."
        : "Local classifier saved. Clients classify on-device and receive the selected provider credential when required.");
      router.refresh();
    }
  }
  ...
  async function testClassifier() {
    ...
    const response = await fetch("/api/admin/classifier/test", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ enabled: true, executionMode, providerId, endpoint: effectiveEndpoint, model, credentialId }),
    });
    const payload = await response.json().catch(() => ({}));
    setTesting(false);
    if (!response.ok) {
  ...
  return (
    <section className="panel" aria-labelledby="classifier-title">
      <div className="panel-header">
        <div><p className="eyebrow">Choose where semantic inference runs</p><h2 id="classifier-title">Workspace classifier</h2></div>
      ...
      <div className="panel-body">
        <form className="settings-form" onSubmit={save}>
          ...
          <fieldset className="execution-mode-field" disabled={!enabled}>
            <legend>Execution and privacy</legend>
            <div className="execution-mode-grid">
              <label className={`mode-option${executionMode === "local" ? " active" : ""}`}>
                <input type="radio" name="executionMode" value="local" checked={executionMode === "local"} onChange={() => setExecutionMode("local")} />
                <span><strong>Local / private</strong><small>Semantic text stays on each client. Hosted provider keys are delivered to that client; localhost models need no key.</small></span>
              </label>
              <label className={`mode-option${executionMode === "managed" ? " active" : ""}`}>
                <input type="radio" name="executionMode" value="managed" checked={executionMode === "managed"} onChange={() => setExecutionMode("managed")} />
                <span><strong>Managed / SaaS</strong><small>Bounded semantic text is sent to Metrune. The provider key stays encrypted in the server vault and is never returned to clients.</small></span>
              </label>
            </div>
          </fieldset>
          <div className="form-grid">
            ...
            <label className="field wide">
              ...
              <small>{executionMode === "managed"
                ? "Used only by the Metrune API. It is never included in client provisioning."
                : "Provisioned to enrolled clients for local provider calls. Use Ollama to keep text local without a provider key."}</small>
            </label>
          </div>
        ...
        {message && <p className="form-ok" role="status">{message}</p>}
        {error && <p className="form-error" role="alert">{error}</p>}
        <p className="panel-note">Response handling is automatic: Metrune uses structured JSON when supported, falls back to prompt-based JSON, and retries one malformed response. The test sends only synthetic text. A remote hosted model cannot both hide its key and receive no inference text; use a local model for that privacy boundary.</p>
      </div>
    </section>
  );
}
...
export function CredentialsPanel({ credentials }: { credentials: ProviderCredential[] }) {
  ...
  return (
    <section className="panel" aria-labelledby="credentials-title">
      <div className="panel-header">
        <div><p className="eyebrow">Stored encrypted, never returned to clients</p><h2 id="credentials-title">Provider credentials</h2></div>
